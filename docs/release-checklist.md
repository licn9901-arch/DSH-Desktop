...
- [ ] `runtime.lock.json` 中当前发布平台的 Node archive/SHA-256、DSH、DSH Market 与 pnpm package lock 完全匹配。
...
- [ ] 9 个托管 bundle、GenUI Skill、Skin Center、关键客户端资产和当前平台 PTY（Windows x64 / macOS x64 / macOS arm64）缺少任一文件时构建失败。
...
- [ ] NSIS 安装包、macOS DMG、`.sha256`、`third-party-licenses.json`、第三方声明和构建摘要齐全。
- [ ] Release 标记为 prerelease，并明确社区非官方、Windows x64/macOS 架构、内置版本和未签名提示。
- [ ] Windows 10 22H2、Windows 11 x64 与 macOS Intel/Apple Silicon 干净环境完成离线首次启动和任务测试。
