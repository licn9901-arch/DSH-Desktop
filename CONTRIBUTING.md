...
在 macOS/Linux shell 或 Windows PowerShell 7 中运行等价命令：
...
修改运行时或打包流程后还应运行 `npm run stage:runtime`、`npm run verify:runtime` 和对应平台验证：Windows 运行冒烟测试，macOS 检查 `.app`/DMG、内置 Node 与 `node-pty` 架构。公开发布前必须在本机完成发布检查表，再推送 `v*` 标签并用 `gh release create --prerelease` 上传校验后的产物。
